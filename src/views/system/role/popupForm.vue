        ...
        <el-scrollbar v-loading="baTable.form.loading" class="ba-table-form-scrollbar">
            ...
            >
                ...
                >
                    ...
                    <el-form-item prop="name" :label="t('system.role.Group name')">
                        <el-input
                            v-model="baTable.form.items!.name"
                            type="string"
                            :placeholder="t('Please input field', { field: t('system.role.Group name') })"
                        ></el-input>
                    </el-form-item>
                    <el-form-item prop="auth" :label="t('system.role.jurisdiction')">
                        <el-tree
                            ref="treeRef"
                            :key="state.treeKey"
...
import { getAdminRules } from '/@/api/system/group';
...
const rules: Partial<Record<string, FormItemRule[]>> = reactive({
    name: [buildValidatorData({ name: 'required', title: t('system.role.Group name') })],
    auth: [
        {
            required: true,
            validator: (rule: any, val: string, callback: Function) => {
                let ids = getCheckeds();
                if (ids.length <= 0) {
                    return callback(new Error(t('Please select field', { field: t('system.role.jurisdiction') })));
                }
                return callback();
            },
        },
    ...
    pid: [
        {
            validator: (rule: any, val: string, callback: Function) => {
                ...
                if (parseInt(val) == parseInt(baTable.form.items!.id)) {
                    return callback(new Error(t('system.role.The parent group cannot be the group itself')));
                }
                return callback();
            },
            trigger: 'blur',
